...
function TransactionManager() {
    ...
    const handleUpdateAmount = (id, amount) => {
        const updatedData = data.map((item) =>
            item.id === id ? { ...item, balance: item.balance + amount, penalty: updatedPenalty } : item
        );
        setData(updatedData);
    };
    ...
    const handleSearch = () => {
        ...
        const filteredData = data.filter((item) =>
            String(item[selectedField]).toLowerCase().includes(searchText.toLowerCase())
        );
        ...
        <>
            ...
            <div className="table-container">
                <div className="header-search">
                    <Typography sx={{ color: '#29aad0', fontSize: '30px', paddingBottom: '20px' }} variant="h4">
                        Transaction Manager
                    </Typography>
                    <div className="search-container">
                        <div>
                            <FormControl sx={{ minWidth: 120, marginRight: '16px' }}>
                                ...
                                <Select sx={{ border: "1px solid Blue", color: "white" }} value={selectedField} onChange={handleFieldChange}>
                                    <MenuItem className='menu-item' value="id">ID</MenuItem>
                                    <MenuItem className='menu-item' value="name">Name</MenuItem>
                                    <MenuItem className='menu-item' value="mobile">Mobile</MenuItem>
                                    <MenuItem className='menu-item' value="balance">Balance</MenuItem>
                                    <MenuItem className='menu-item' value="penalty">Penalty</MenuItem>
                                </Select>
                            </FormControl>
                        </div>
                        ...
                        />
                        <Button sx={{ color:'white', background: 'transparent', border: "1px solid blue" }} variant="contained" onClick={handleSearch}>
                            Search
                        </Button>
                    </div>
                </div>

                <TableContainer sx={{ background: 'transparent', border: '1px solid blue' }} component={Paper}>
                                    ...
                                    <TableCell sx={{ textAlign: 'center', color: 'red' }}>
                                        {item.id === updatedPenaltyId ? (
                                            <TextField
                                                ...
                                                onChange={(e) => setUpdatedPenalty(Number(e.target.value))}
                                            />
                                        ) : (
                                            item.penalty
                                        )}
                                    </TableCell>
                                    <TableCell sx={{ textAlign: 'center', borderBottom: 'transparent' }}>
                                        {item.id === updatedPenaltyId ? (
                                            <div className='button-update'>
                                            <Button
                                                className='Update-button'
                                                sx={{ textAlign: 'center', background: "transparent", color: "lime", border: "1px solid lime" }}
                                                variant="contained"
                                                color="primary"
                                                onClick={() => {
                                            ...
                                            <div className="penalty-button">
                                                <Button variant="outlined" onClick={() => setUpdatedPenaltyId(item.id)}>
                                                    Edit Penalty
                                                </Button>
                                            </div>
                                        )}
                ...
                <TablePagination sx={{ color: "white", marginTop: "20px" }}
